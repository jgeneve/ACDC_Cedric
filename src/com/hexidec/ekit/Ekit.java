...
public class Ekit extends JFrame implements WindowListener
{
	...
	public Ekit(String sDocument, String sStyleSheet, String sRawDocument, URL urlStyleSheet, boolean includeToolBar, boolean showViewSource, boolean showMenuIcons, boolean editModeExclusive, String sLanguage, String sCountry, boolean base64, boolean debugMode, boolean useSpellChecker, boolean multiBar, boolean enterBreak)
	{
		...
		if(includeToolBar)
		{
			if(multiBar)
			{
				...
				gbc.gridy 	   = 1;
				this.getContentPane().add(this.createJPanelTitle(), gbc);
				
				gbc.gridy 	   = 2;
				this.getContentPane().add(this.createJPanelCategory(), gbc);
				
				gbc.gridy 	   = 3;
				this.getContentPane().add(this.createJPanelAuthor(), gbc);
				...
				gbc.weighty    = 0.0;
				gbc.gridy	   = 8;
				this.getContentPane().add(this.createButtonSubmit(), gbc);
				
			}
			else
			{				
				this.getContentPane().setLayout(new BorderLayout());
				this.getContentPane().add(ekitCore, BorderLayout.SOUTH);
	...
	public JPanel createJPanelTitle() {
		...
		jPanelTitle.setBorder(new EmptyBorder(5, 5, 5, 5));
		
		JLabel jLabelTitle = new JLabel("Titre");
		jLabelTitle.setPreferredSize(new Dimension(60, 100));
		
		jTextFieldTitle = new JTextField();
		jTextFieldTitle.setMinimumSize(new Dimension(getWidth(), 25));
		
		jPanelTitle.add(jLabelTitle, BorderLayout.WEST);
		jPanelTitle.add(jTextFieldTitle, BorderLayout.CENTER);
		
		return jPanelTitle;
	}
	
	public JPanel createJPanelCategory() 
	{
		JPanel jPanelCategory = new JPanel();
		jPanelCategory.setLayout(new BoxLayout(jPanelCategory, BoxLayout.X_AXIS));
		jPanelCategory.setBorder(new EmptyBorder(5, 5, 5, 5));
		
		JLabel jLabelCategory = new JLabel("Catégorie");
		jLabelCategory.setPreferredSize(new Dimension(60, 100));
		...
		jPanelCategory.add(jComboBoxCategory);
		jPanelCategory.add(jButtonNoFocusAddCategory);
		jPanelCategory.add(Box.createRigidArea(new Dimension(5, 0)));
		jPanelCategory.add(jButtonNoFocusRemoveCategory);
		
		jButtonNoFocusAddCategory.addActionListener(new ActionListener() {
			
			@Override
			public void actionPerformed(ActionEvent e) {
				new UserInputAddCategory(ekitCore, "Insertion d'une catégorie", true, jComboBoxCategory);
			}
		});
		
		jButtonNoFocusRemoveCategory.addActionListener(new ActionListener() {
			
			@Override
			public void actionPerformed(ActionEvent e) {
				new UserDeleteCategory(ekitCore, "Suppression d'une catégorie", true, jComboBoxCategory);				
			}
		});
	...
	public JPanel createJPanelAuthor() {
		JPanel jPanelAuthor = new JPanel(new BorderLayout());
		jPanelAuthor.setBorder(new EmptyBorder(5, 5, 5, 5));
		
		JLabel jLabelAuthor = new JLabel("Auteur");
		jLabelAuthor.setPreferredSize(new Dimension(60, 100));
		
		jTextFieldAuthor = new JTextField();
		jTextFieldAuthor.setMinimumSize(new Dimension(getWidth(), 25));
		
		jPanelAuthor.add(jLabelAuthor, BorderLayout.WEST);
		jPanelAuthor.add(jTextFieldAuthor, BorderLayout.CENTER);
	...
	public JButton createButtonSubmit() {
		...
		jbtnSubmitForm.addActionListener(new ActionListener() {
			
			@Override
			public void actionPerformed(ActionEvent e) {
				if (isFormValid()) {
					Post post = new Post(jTextFieldTitle.getText(), 
					...
					String gitDirectory = PropertiesAccess.getInstance().getLocalRepository();
		...
		return jbtnSubmitForm;
	}
	
	private boolean isFormValid() {
		List<String> errorsEmpty = new ArrayList<>();
		if (jTextFieldTitle.getText().isEmpty()) {
			errorsEmpty.add("- Veuillez entrer un titre.");
		}
		if (jComboBoxCategory.getSelectedItem() == null) {
			errorsEmpty.add("\n- Veuillez créer une catégorie.");
		}
		if (jTextFieldAuthor.getText().isEmpty()) {
			errorsEmpty.add("\n- Veuillez entrer au moins un auteur.");
		}
		
		if (!errorsEmpty.isEmpty()) {
			new DialogInvalidForm(ekitCore, "Formulaire invalide", true, errorsEmpty);
			return false;
		}
		return true;
	}
